#!/usr/bin/env python3
"""Run RTL co-simulation for HWC channel-parallel four-stage HLS projects.

This gate consumes ``fourstage_hwc_parallel_hls_summary.json``.  It does not
regenerate architectures, retrain checkpoints, alter quantization, or route the
design.  It only opens the already-synthesized Vitis HLS projects and runs
``cosim_design`` against the generated full-network testbench.
...
DEFAULT_HLS_SUMMARY = ARTIFACT_ROOT / "fourstage_hwc_parallel_hls_summary.json"
DEFAULT_SUMMARY = ARTIFACT_ROOT / "fourstage_hwc_parallel_rtl_cosim_summary.json"
...
def run_vitis_cosim(
    *, vitis_hls: Path, hls_dir: Path, timeout_minutes: int
    ...
    log_path = hls_dir / "vitis_hls_hwc_parallel_rtl_cosim.log"
    command = ["cmd", "/c", str(vitis_hls), "-f", "run_rtl_cosim.tcl"]
...
def classify_failure(
    *,
    ...
    if status == "PASS":
        return None
    if timed_out:
        return "HWC_PARALLEL_RTL_COSIM_TIMEOUT"
    if not isinstance(result_payload, dict):
        return "HWC_PARALLEL_RTL_COSIM_NO_RESULT_JSON"
    if result_payload.get("status") != "PASS" or result_payload.get("mismatch_count") != 0:
        return "HWC_PARALLEL_RTL_COSIM_OUTPUT_MISMATCH"
    if log_path is not None and log_path.is_file():
        log_text = log_path.read_text(encoding="utf-8", errors="replace")
        if "ERROR:" in log_text:
            return "HWC_PARALLEL_RTL_COSIM_TOOL_ERROR"
    if returncode != 0:
        return "HWC_PARALLEL_RTL_COSIM_RETURNCODE_NONZERO"
    return "HWC_PARALLEL_RTL_COSIM_FAIL"
...
def main() -> int:
    ...
    if hls_summary.get("status") != "PASS":
        raise RuntimeError("HWC channel-parallel HLS gate must PASS before RTL co-sim")
    ...
    if not candidates:
        raise RuntimeError("no candidates selected for HWC parallel RTL co-sim")
    ...
    if args.run_cosim and not vitis_hls.is_file():
        payload = {
            ...
            "status": "BLOCKED",
            "gate": "full_network_hwc_parallel_rtl_cosim",
    ...
    formal_scope = (
        ...
        and int(hls_summary.get("candidate_count") or len(rows)) == formal_candidate_count
    )
